...
public:
    TestEntity(const std::string& name) : m_name(name) {}
...
int main() {
    ...
    TestEntity player("Player");
    TestEntity enemy("Enemy");
    TestEntity wall("Wall");
    
    // Initialize entities (subscribe to events)
    player.initialize();
    enemy.initialize();
    wall.initialize();
    ...
    EventSystem::getInstance().publish("damage", damage1, &enemy);
    ...
    // Test unsubscribing by cleaning up the wall entity
    wall.cleanup();
